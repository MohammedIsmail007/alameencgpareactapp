...
const Ecesem5r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/ecesem4"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/ecesem4/${regno}`
      );
  ...
  var ecevale5 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var ecesem5 = [3, 3, 4, 4, 3, 3, 0];
  var ecesum5 = 0;
  var ecegpa5 = [];
  var ecetot5 = 0;
  var mulecesum5 = 0;
  ...
  function get5() {
    var ses = document.getElementsByClassName("selectecesem5");
    var creditregece4 = 0; // Initialize creditreg to 0
    ecesum5 = 0; // Ensure sum1 is reset
    ecegpa5 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      ecegpa5.push(ecevale5[val4] * ecesem5[i]);
      ecesum5 += ecevale5[val4] * ecesem5[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregece4 += ecesem5[i];
      }
    }
    ...
    ecetot5 = creditregece4 === 0 ? 0 : (mulecesum5 / creditregece4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregece5").value = creditregece4;
    document.getElementById("totsumvalueece5").value = ecesum5;
    document.getElementById("ecegpa5").value = ecetot5;
  }

  // ////console.log(studentData.totcredit);
  function sem5cgpacalc() {
    const prevcredit = document.getElementById("eceprevcredit5").value;
    const sem4credit =
      parseFloat(studentData.ecetotcredit) + parseFloat(prevcredit);
    const sem4totsum = parseFloat(studentData.ecetotsum);
    const sem5totsum = document.getElementById("totsumvalueece5").value;
    const sem5credit = document.getElementById("totcreditregece5").value;
    ...
    document.getElementById("totcecegpa5").value = cgpasem5total.toFixed(2);
    ...
    //console.log(cgpasem5total);
    document.getElementById("totalecesum5").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditece5").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/ecesem5", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectecesem5"
