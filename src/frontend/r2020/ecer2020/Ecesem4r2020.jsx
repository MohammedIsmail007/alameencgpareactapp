...
const Ecesem3r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/ecesem3"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/ecesem3/${regno}`
      );
  ...
  var ecevale4 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var ecesem4 = [3, 4, 3, 4, 3, 1.0, 1.5, 1.5, 3];
  var ecesum4 = 0;
  var ecegpa4 = [];
  var ecetot4 = 0;
  var mulecesum4 = 0;
  ...
  function get4() {
    var ses = document.getElementsByClassName("selectecesem4");
    var creditregece4 = 0; // Initialize creditreg to 0
    ecesum4 = 0; // Ensure sum1 is reset
    ecegpa4 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      ecegpa4.push(ecevale4[val4] * ecesem4[i]);
      ecesum4 += ecevale4[val4] * ecesem4[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregece4 += ecesem4[i];
      }
    }
    ...
    ecetot4 = creditregece4 === 0 ? 0 : (mulecesum4 / creditregece4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregece4").value = creditregece4;
    document.getElementById("totsumvalueece4").value = ecesum4;
    document.getElementById("ecegpa4").value = ecetot4;
  }

  // //console.log(studentData.totcredit);
  function sem4cgpacalc() {
    const prevcredit = document.getElementById("eceprevcredit4").value;
    const sem3credit =
      parseFloat(studentData.ecetotcredit) + parseFloat(prevcredit);
    const sem3totsum = parseFloat(studentData.ecetotsum);
    const sem4totsum = document.getElementById("totsumvalueece4").value;
    const sem4credit = document.getElementById("totcreditregece4").value;
    ...
    document.getElementById("totcecegpa4").value = cgpasem4total.toFixed(2);
    ...
    //console.log(cgpasem4total);
    document.getElementById("totalecesum4").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditece4").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/ecesem4", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectecesem4"
