...
const Ecesem6r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/ecesem5"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/ecesem5/${regno}`
      );
  ...
  var ecevale6 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var ecesem6 = [4, 3, 3, 3, 4, 4, 0];
  var ecesum6 = 0;
  var ecegpa6 = [];
  var ecetot6 = 0;
  var mulecesum6 = 0;
  ...
  function get5() {
    var ses = document.getElementsByClassName("selectecesem6");
    var creditregece6 = 0; // Initialize creditreg to 0
    ecesum6 = 0; // Ensure sum1 is reset
    ecegpa6 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val6 = ses[i].options[ses[i].selectedIndex].value;
      ecegpa6.push(ecevale6[val6] * ecesem6[i]);
      ecesum6 += ecevale6[val6] * ecesem6[i];

      // Update creditreg only if the selected option is not "other"
      if (val6 !== "other") {
        creditregece6 += ecesem6[i];
      }
    }
    ...
    ecetot6 = creditregece6 === 0 ? 0 : (mulecesum6 / creditregece6).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregece6").value = creditregece6;
    document.getElementById("totsumvalueece6").value = ecesum6;
    document.getElementById("ecegpa6").value = ecetot6;
  }

  // ////console.log(studentData.totcredit);
  function sem5cgpacalc() {
    const prevcredit = document.getElementById("eceprevcredit6").value;
    const sem5credit =
      parseFloat(studentData.ecetotcredit) + parseFloat(prevcredit);
    const sem5totsum = parseFloat(studentData.ecetotsum);
    const sem6totsum = document.getElementById("totsumvalueece6").value;
    const sem6credit = document.getElementById("totcreditregece6").value;
    ...
    document.getElementById("totcecegpa6").value = cgpasem6total.toFixed(2);
    ...
    //console.log(cgpasem5total);
    document.getElementById("totalecesum6").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditece6").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/ecesem6", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectecesem6"
