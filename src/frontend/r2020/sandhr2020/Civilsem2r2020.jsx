...
const Civilsem2r2020 = () => {
  ...
  function get2() {
    var ses = document.getElementsByClassName("selectcivilsem2");
    ...
    for (var i = 0; i < ses.length; i++) {
      ...
      gpa2.push(vale2[val2] * civilsem22[i]);
      sum2 += vale2[val2] * civilsem22[i];

      // Update creditreg only if the selected option is not "other"
      if (val2 !== "other") {
        creditreg2 += civilsem22[i];
      }
    }
  ...
  function civilsem2cgpacalc() {
    const sem1credit =
      parseFloat(studentData.totcredit) + parseFloat(studentData.prevcredit);
    const sem1totsum = parseFloat(studentData.totsum);
    const civilsem2totsum = document.getElementById("totsumvalue2").value;
    const civilsem2credit = document.getElementById("totcreditreg2").value;

    const totalsum =
      parseFloat(sem1totsum) +
      parseFloat(civilsem2totsum) +
      parseFloat(arreartotalgrades);
    const totalcredit =
      parseFloat(sem1credit) +
      parseFloat(civilsem2credit) +
      parseFloat(arreartotalcreditscore);
    const cgpacivilsem2total = totalsum / totalcredit;
    document.getElementById("totcgpa2").value = cgpacivilsem2total.toFixed(2);
    // //console.log(sem1credit);
    // //console.log(sem1totsum);
    // //console.log(civilsem2totsum);
    // //console.log(arreartotalcreditscore);
    // //console.log(arreartotalgrades);
    // //console.log(totalsum);
    // //console.log(totalcredit);
    // //console.log(cgpacivilsem2total);
    // //console.log(arreartotalcreditscore);
    // //console.log(arreartotalgrades);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://alameencgpareactappserver.onrender.com/civilsem2",
        data
      );
