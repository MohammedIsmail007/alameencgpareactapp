...
const Mechsem2r2020 = () => {
  ...
  function get2() {
    var ses = document.getElementsByClassName("selectmechsem2");
    var creditreg2 = 0; // Initialize creditreg to 0
    sum2 = 0; // Ensure sum1 is reset
    gpa2 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val2 = ses[i].options[ses[i].selectedIndex].value;
      gpa2.push(vale2[val2] * mechsem22[i]);
      sum2 += vale2[val2] * mechsem22[i];

      // Update creditreg only if the selected option is not "other"
      if (val2 !== "other") {
        creditreg2 += mechsem22[i];
      }
    }
    ...
    tot2 = creditreg2 === 0 ? 0 : (mulsum2 / creditreg2).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditreg2").value = creditreg2;
    document.getElementById("totsumvalue2").value = sum2;
    document.getElementById("gpa2").value = tot2;
  }

  // //console.log(studentData.totcredit);
  function mechsem2cgpacalc() {
    const sem1credit =
      parseFloat(studentData.totcredit) + parseFloat(studentData.prevcredit);
    const sem1totsum = parseFloat(studentData.totsum);
    const mechsem2totsum = document.getElementById("totsumvalue2").value;
    const mechsem2credit = document.getElementById("totcreditreg2").value;

    const totalsum =
      parseFloat(sem1totsum) +
      parseFloat(mechsem2totsum) +
      parseFloat(arreartotalgrades);
    const totalcredit =
      parseFloat(sem1credit) +
      parseFloat(mechsem2credit) +
      parseFloat(arreartotalcreditscore);
    const cgpamechsem2total = totalsum / totalcredit;
    document.getElementById("totcgpa2").value = cgpamechsem2total.toFixed(2);
    // //console.log(sem1credit);
    // //console.log(sem1totsum);
    // //console.log(mechsem2totsum);
    // //console.log(arreartotalcreditscore);
    // //console.log(arreartotalgrades);
    // //console.log(totalsum);
    // //console.log(totalcredit);
    // //console.log(cgpamechsem2total);
    // //console.log(arreartotalcreditscore);
    // //console.log(arreartotalgrades);
    document.getElementById("totalsum2").value = totalsum.toFixed(3);
    document.getElementById("totalcredit2").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/mechsem2", data);
              ...
              <div>
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group m-3 p-3">
                            <select
                              className="form-select selectmechsem2"
                ...
                >
                  <h2 className="text-xl mb-4 font-bold text-blue-900 text-center">
                    IF YOU HAVE CLEARED CURRENT SEM ARREARS <br />
                    THEN SELECT NUMBER OF ARREARS
                  </h2>
                  <select
                ...
                >
                  <div className="card  shadow-2xl  p-3 rounded-lg">
                    <div className="card-body flex flex-col flex-wrap justify-center items-center gap-4 ">
                      ...
                      />
                      <br />
                      {/* student detail fetching section to calculate cgpa */}
                      <div
                        className={`${
                          printaction
                            ? "printaction"
                      ...
                      <input
                        ...
                        readOnly
                      />
                      {/* {to add total values in sem 2 table database} */}
                      <input
                        type="text"
                        className={`${
                          printaction
                      ...
                      />
                      {/* {-------------------------------------------------------------------} */}
                      <div className="flex flex-wrap flex-col p-3 m-2 gap-4 justify-center items-center container">
