...
const Csesem5r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/csesem4"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/csesem4/${regno}`
      );
  ...
  var csevale5 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var csesem5 = [3, 3, 4, 4, 3, 3, 0];
  var csesum5 = 0;
  var csegpa5 = [];
  var csetot5 = 0;
  var mulcsesum5 = 0;
  ...
  function get5() {
    var ses = document.getElementsByClassName("selectcsesem5");
    var creditregcse4 = 0; // Initialize creditreg to 0
    csesum5 = 0; // Ensure sum1 is reset
    csegpa5 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      csegpa5.push(csevale5[val4] * csesem5[i]);
      csesum5 += csevale5[val4] * csesem5[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregcse4 += csesem5[i];
      }
    }
    ...
    csetot5 = creditregcse4 === 0 ? 0 : (mulcsesum5 / creditregcse4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregcse5").value = creditregcse4;
    document.getElementById("totsumvaluecse5").value = csesum5;
    document.getElementById("csegpa5").value = csetot5;
  }

  // ////console.log(studentData.totcredit);
  function sem5cgpacalc() {
    const prevcredit = document.getElementById("cseprevcredit5").value;
    const sem4credit =
      parseFloat(studentData.csetotcredit) + parseFloat(prevcredit);
    const sem4totsum = parseFloat(studentData.csetotsum);
    const sem5totsum = document.getElementById("totsumvaluecse5").value;
    const sem5credit = document.getElementById("totcreditregcse5").value;
    ...
    document.getElementById("totccsegpa5").value = cgpasem5total.toFixed(2);
    ...
    //console.log(cgpasem5total);
    document.getElementById("totalcsesum5").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditcse5").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/csesem5", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectcsesem5"
