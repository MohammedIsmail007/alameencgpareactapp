...
const Csesem4r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/csesem3"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/csesem3/${regno}`
      );
  ...
  var csevale4 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var csesem4 = [3, 4, 3, 4, 3, 1.0, 1.5, 1.5, 3];
  var csesum4 = 0;
  var csegpa4 = [];
  var csetot4 = 0;
  var mulcsesum4 = 0;
  ...
  function get4() {
    var ses = document.getElementsByClassName("selectcsesem4");
    var creditregcse4 = 0; // Initialize creditreg to 0
    csesum4 = 0; // Ensure sum1 is reset
    csegpa4 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      csegpa4.push(csevale4[val4] * csesem4[i]);
      csesum4 += csevale4[val4] * csesem4[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregcse4 += csesem4[i];
      }
    }
    ...
    csetot4 = creditregcse4 === 0 ? 0 : (mulcsesum4 / creditregcse4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregcse4").value = creditregcse4;
    document.getElementById("totsumvaluecse4").value = csesum4;
    document.getElementById("csegpa4").value = csetot4;
  }

  // //console.log(studentData.totcredit);
  function sem4cgpacalc() {
    const prevcredit = document.getElementById("cseprevcredit4").value;
    const sem3credit =
      parseFloat(studentData.csetotcredit) + parseFloat(prevcredit);
    const sem3totsum = parseFloat(studentData.csetotsum);
    const sem4totsum = document.getElementById("totsumvaluecse4").value;
    const sem4credit = document.getElementById("totcreditregcse4").value;
    ...
    document.getElementById("totccsegpa4").value = cgpasem4total.toFixed(2);
    ...
    //console.log(cgpasem4total);
    document.getElementById("totalcsesum4").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditcse4").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/csesem4", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectcsesem4"
