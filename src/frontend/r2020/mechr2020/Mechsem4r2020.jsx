...
const Mechsem4r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/mechsem3"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/mechsem3/${regno}`
      );
  ...
  var mechvale4 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var mechsem4 = [3, 4, 3, 4, 4, 1.0, 1.0, 3];
  var mechsum4 = 0;
  var mechgpa4 = [];
  var mechtot4 = 0;
  var mulmechsum4 = 0;
  ...
  function get4() {
    var ses = document.getElementsByClassName("selectmechsem4");
    var creditregmech4 = 0; // Initialize creditreg to 0
    mechsum4 = 0; // Ensure sum1 is reset
    mechgpa4 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      mechgpa4.push(mechvale4[val4] * mechsem4[i]);
      mechsum4 += mechvale4[val4] * mechsem4[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregmech4 += mechsem4[i];
      }
    }

    // Calculate the GPA
    mulmechsum4 = mechsum4;
    //const prevcredit = parseFloat(studentData.prevcredit);

    ////console.log(creditreg);
    mechtot4 =
      creditregmech4 === 0 ? 0 : (mulmechsum4 / creditregmech4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregmech4").value = creditregmech4;
    document.getElementById("totsumvaluemech4").value = mechsum4;
    document.getElementById("mechgpa4").value = mechtot4;
  }

  // //console.log(studentData.totcredit);
  function sem4cgpacalc() {
    const prevcredit = document.getElementById("mechprevcredit4").value;
    const sem3credit =
      parseFloat(studentData.mechtotcredit) + parseFloat(prevcredit);
    const sem3totsum = parseFloat(studentData.mechtotsum);
    const sem4totsum = document.getElementById("totsumvaluemech4").value;
    const sem4credit = document.getElementById("totcreditregmech4").value;
    ...
    document.getElementById("totcmechgpa4").value = cgpasem4total.toFixed(2);
    ...
    //console.log(cgpasem4total);
    document.getElementById("totalmechsum4").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditmech4").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/mechsem4", data);
                ...
                <div className="justify-center flex flex-wrap p-3  gap-3">
                  <table className="table-bordered border-2 rounded-sm gap-5 bg-white text-center">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectmechsem4"
                      ...
                      <tr>
                        <th>Manufacturing Technology - II</th>
                        <th>3</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Theory of Machines Laboratory</th>
                        <th>1.0</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Manufacturing Process Laboratory</th>
                        <th>1.0</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
