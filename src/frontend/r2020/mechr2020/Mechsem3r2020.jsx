...
const Mechsem3r2020 = () => {
  ...
  var mechvale3 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var mechsem3 = [4, 3, 3, 4, 4, 3, 1.0, 1.0, 1.0, 0];
  var mechsum3 = 0;
  var mechgpa3 = [];
  var mechtot3 = 0;
  var mulmechsum3 = 0;
  var creditregmech3 = 0;
  function get3() {
    var ses = document.getElementsByClassName("selectmechsem3");
    var creditregmech3 = 0; // Initialize creditreg to 0
    mechsum3 = 0; // Ensure sum1 is reset
    mechgpa3 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val2 = ses[i].options[ses[i].selectedIndex].value;
      mechgpa3.push(mechvale3[val2] * mechsem3[i]);
      mechsum3 += mechvale3[val2] * mechsem3[i];

      // Update creditreg only if the selected option is not "other"
      if (val2 !== "other") {
        creditregmech3 += mechsem3[i];
      }
    }

    // Calculate the GPA
    mulmechsum3 = mechsum3;
    //const prevcredit = parseFloat(studentData.prevcredit);

    ////console.log(creditreg);
    mechtot3 =
      creditregmech3 === 0 ? 0 : (mulmechsum3 / creditregmech3).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregmech3").value = creditregmech3;
    document.getElementById("totsumvaluemech3").value = mechsum3;
    document.getElementById("mechgpa3").value = mechtot3;
  }

  // //console.log(studentData.totcredit);
  function sem3cgpacalc() {
    const prevcredit = document.getElementById("mechprevcredit3").value;
    const sem2credit =
      parseFloat(studentData.totcredit2) + parseFloat(prevcredit);
    const sem2totsum = parseFloat(studentData.totsum2);
    const sem3totsum = document.getElementById("totsumvaluemech3").value;
    const sem3credit = document.getElementById("totcreditregmech3").value;
    ...
    document.getElementById("totcmechgpa3").value = cgpasem2total.toFixed(2);
    ...
    // //console.log(cgpasem2total);
    document.getElementById("totalmechsum3").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditmech3").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/mechsem3", data);
                ...
                <div className="justify-center flex flex-wrap p-3  gap-3">
                  ...
                  <table className="table-bordered border-2 rounded-sm gap-5 bg-white text-center">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectmechsem3"
                      ...
                      <tr>
                        <th>Basics of Electronics Engineering </th>
                        <th>3</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Manufacturing Technology I</th>
                        <th>3</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Fluid Mechanics and Machinery</th>
                        <th>4</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Fluid Mechanics Laboratory</th>
                        <th>1.0</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
