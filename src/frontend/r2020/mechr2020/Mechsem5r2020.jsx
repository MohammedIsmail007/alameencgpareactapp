...
const Mechsem5r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/mechsem4"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/mechsem4/${regno}`
      );
  ...
  var mechvale5 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var mechsem5 = [3, 3, 4, 4, 3, 3, 0];
  var mechsum5 = 0;
  var mechgpa5 = [];
  var mechtot5 = 0;
  var mulmechsum5 = 0;
  ...
  function get5() {
    var ses = document.getElementsByClassName("selectmechsem5");
    var creditregmech4 = 0; // Initialize creditreg to 0
    mechsum5 = 0; // Ensure sum1 is reset
    mechgpa5 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      mechgpa5.push(mechvale5[val4] * mechsem5[i]);
      mechsum5 += mechvale5[val4] * mechsem5[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregmech4 += mechsem5[i];
      }
    }

    // Calculate the GPA
    mulmechsum5 = mechsum5;
    //const prevcredit = parseFloat(studentData.prevcredit);

    //////console.log(creditreg);
    mechtot5 =
      creditregmech4 === 0 ? 0 : (mulmechsum5 / creditregmech4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregmech5").value = creditregmech4;
    document.getElementById("totsumvaluemech5").value = mechsum5;
    document.getElementById("mechgpa5").value = mechtot5;
  }

  // ////console.log(studentData.totcredit);
  function sem5cgpacalc() {
    const prevcredit = document.getElementById("mechprevcredit5").value;
    const sem4credit =
      parseFloat(studentData.mechtotcredit) + parseFloat(prevcredit);
    const sem4totsum = parseFloat(studentData.mechtotsum);
    const sem5totsum = document.getElementById("totsumvaluemech5").value;
    const sem5credit = document.getElementById("totcreditregmech5").value;
    ...
    document.getElementById("totcmechgpa5").value = cgpasem5total.toFixed(2);
    ...
    //console.log(cgpasem5total);
    document.getElementById("totalmechsum5").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditmech5").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/mechsem5", data);
              ...
              <div>
                ...
                <div className="justify-center flex flex-wrap p-3  gap-3">
                  ...
                  <table className="table-bordered border-2 rounded-sm gap-5 bg-white text-center">
                    ...
                    <tbody>
                      <tr>
                        <th>Environmental and Agriculture Engineering</th>
                        <th>3</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectmechsem5"
                      ...
                      <tr>
                        <th>Internal Combustion Engines</th>
                        <th>4</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Design of Machine Elements</th>
                        <th>0</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                      ...
                      <tr>
                        <th>Thermal Engineering Laboratory</th>
                        <th>0</th>
                        <td>
                          <div className="input-group mb-3">
                            <select
                  ...
                  <div className="card  shadow-2xl  p-3 rounded-lg">
                    ...
                    {regnos.map((student) => (
                      ...
                      <input
                        ...
                        readOnly
                      />
                      {/* {to add total values in mech sem 3 table database} */}
                      <input
                        type="text"
                        className={`${
                          printaction
