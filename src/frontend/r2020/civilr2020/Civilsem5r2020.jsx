...
const Civilsem5r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/civilsem4"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/civilsem4/${regno}`
      );
  ...
  function get5() {
    var ses = document.getElementsByClassName("selectcivilsem5");
    var creditregcivil4 = 0; // Initialize creditreg to 0
    civilsum5 = 0; // Ensure sum1 is reset
    civilgpa5 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      civilgpa5.push(civilvale5[val4] * civilsem5[i]);
      civilsum5 += civilvale5[val4] * civilsem5[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregcivil4 += civilsem5[i];
      }
    }

    // Calculate the GPA
    mulcivilsum5 = civilsum5;
    //const prevcredit = parseFloat(studentData.prevcredit);

    //////console.log(creditreg);
    civiltot5 =
      creditregcivil4 === 0 ? 0 : (mulcivilsum5 / creditregcivil4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregcivil5").value = creditregcivil4;
    document.getElementById("totsumvaluecivil5").value = civilsum5;
    document.getElementById("civilgpa5").value = civiltot5;
  }

  // ////console.log(studentData.totcredit);
  function sem5cgpacalc() {
    const prevcredit = document.getElementById("civilprevcredit5").value;
    const sem4credit =
      parseFloat(studentData.civiltotcredit) + parseFloat(prevcredit);
    const sem4totsum = parseFloat(studentData.civiltotsum);
    const sem5totsum = document.getElementById("totsumvaluecivil5").value;
    const sem5credit = document.getElementById("totcreditregcivil5").value;
    ...
    document.getElementById("totccivilgpa5").value = cgpasem5total.toFixed(2);
    ...
    //console.log(cgpasem5total);
    document.getElementById("totalcivilsum5").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditcivil5").value = totalcredit;
  }
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectcivilsem5"
                  ...
                  <div className="card  shadow-2xl  p-3 rounded-lg">
                    <div className="card-body flex flex-col flex-wrap justify-center items-center gap-4 ">
                      ...
                      <input
                        ...
                        readOnly
                      />
                      {/* {to add total values in civil sem 3 table database} */}
                      <input
                        type="text"
                        className={`${
                          printaction
