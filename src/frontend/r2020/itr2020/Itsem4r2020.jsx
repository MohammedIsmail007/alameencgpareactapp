...
const Itsem4r2020 = () => {
  ...
  const fetchRegnos = async () => {
    try {
      const response = await axios.get("http://localhost:5000/itsem3"); // Update this endpoint as necessary
  ...
  const fetchStudentData = async (regno) => {
    ...
    setError(null);
    try {
      const response = await axios.get(`http://localhost:5000/itsem3/${regno}`);
  ...
  var itvale4 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var itsem4 = [3, 4, 3, 4, 3, 1.0, 1.5, 1.5, 3];
  var itsum4 = 0;
  var itgpa4 = [];
  var ittot4 = 0;
  var mulitsum4 = 0;
  ...
  function get4() {
    var ses = document.getElementsByClassName("selectitsem4");
    var creditregit4 = 0; // Initialize creditreg to 0
    itsum4 = 0; // Ensure sum1 is reset
    itgpa4 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val4 = ses[i].options[ses[i].selectedIndex].value;
      itgpa4.push(itvale4[val4] * itsem4[i]);
      itsum4 += itvale4[val4] * itsem4[i];

      // Update creditreg only if the selected option is not "other"
      if (val4 !== "other") {
        creditregit4 += itsem4[i];
      }
    }
    ...
    ittot4 = creditregit4 === 0 ? 0 : (mulitsum4 / creditregit4).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregit4").value = creditregit4;
    document.getElementById("totsumvalueit4").value = itsum4;
    document.getElementById("itgpa4").value = ittot4;
  }

  // //console.log(studentData.totcredit);
  function sem4cgpacalc() {
    const prevcredit = document.getElementById("itprevcredit4").value;
    const sem3credit =
      parseFloat(studentData.ittotcredit) + parseFloat(prevcredit);
    const sem3totsum = parseFloat(studentData.ittotsum);
    const sem4totsum = document.getElementById("totsumvalueit4").value;
    const sem4credit = document.getElementById("totcreditregit4").value;
    ...
    document.getElementById("totcitgpa4").value = cgpasem4total.toFixed(2);
    ...
    //console.log(cgpasem4total);
    document.getElementById("totalitsum4").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditit4").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/itsem4", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectitsem4"
