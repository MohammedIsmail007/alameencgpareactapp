...
const Itsem3r2020 = () => {
  ...
  var itvale3 = { O: 10, Aplus: 9, A: 8, Bplus: 7, B: 6, C: 5, other: 0 };
  var itsem3 = [4, 4, 3, 3, 3, 1.5, 1.5, 1.5, 0, 0];
  var itsum3 = 0;
  var itgpa3 = [];
  var ittot3 = 0;
  var mulitsum3 = 0;
  var creditregit3 = 0;
  function get3() {
    var ses = document.getElementsByClassName("selectitsem3");
    var creditregit3 = 0; // Initialize creditreg to 0
    itsum3 = 0; // Ensure sum1 is reset
    itgpa3 = []; // Ensure gpa1 is reset

    for (var i = 0; i < ses.length; i++) {
      var val2 = ses[i].options[ses[i].selectedIndex].value;
      itgpa3.push(itvale3[val2] * itsem3[i]);
      itsum3 += itvale3[val2] * itsem3[i];

      // Update creditreg only if the selected option is not "other"
      if (val2 !== "other") {
        creditregit3 += itsem3[i];
      }
    }
    ...
    ittot3 = creditregit3 === 0 ? 0 : (mulitsum3 / creditregit3).toFixed(3);

    // Update the form fields
    document.getElementById("totcreditregit3").value = creditregit3;
    document.getElementById("totsumvalueit3").value = itsum3;
    document.getElementById("itgpa3").value = ittot3;
  }

  // //console.log(studentData.totcredit);
  function sem3cgpacalc() {
    const prevcredit = document.getElementById("itprevcredit3").value;
    const sem2credit =
      parseFloat(studentData.totcredit2) + parseFloat(prevcredit);
    const sem2totsum = parseFloat(studentData.totsum2);
    const sem3totsum = document.getElementById("totsumvalueit3").value;
    const sem3credit = document.getElementById("totcreditregit3").value;
    ...
    document.getElementById("totcitgpa3").value = cgpasem2total.toFixed(2);
    ...
    // //console.log(cgpasem2total);
    document.getElementById("totalitsum3").value =
      parseFloat(totalsum).toFixed(3);
    document.getElementById("totalcreditit3").value = totalcredit;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/itsem3", data);
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="input-group mb-3">
                            <select
                              className="form-select selectitsem3"
